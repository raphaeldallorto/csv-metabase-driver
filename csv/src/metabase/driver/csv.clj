...
;; Remova estes métodos obsoletos
...
(defmethod sql-jdbc.conn/connection-details->spec :csv
  ...
  (let [strHttp (when (is-http csv)
                  ":class:br.markenson.com.csvjdbc4metabase.readers.HttpCSVReader")
        customBaseUrl (if (is-http csv)
                        (str "&customBaseUrl=" csv)
                        "")]

    (merge
     {:classname   "org.relique.jdbc.csv.CsvDriver"
      :subprotocol (str "relique:csv" strHttp)
      :subname     (str (if (is-http csv) "" csv)
                        "?separator=" separator
                        "&charset=" charset
                        customBaseUrl
                        advanced)}
